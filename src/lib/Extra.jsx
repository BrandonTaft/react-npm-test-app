  ...
  useEffect(() => {
    ...
    if (JSON.stringify(cacheRef.current) !== JSON.stringify(list)) {
      ...
      if (cleanList.current) {
        for (let i = 0; i < cleanList.current.length; i++) {
          const item = cleanList.current[i]
...
console.log("RAN@@@")
  ...
  }, [list, getPropValue, highlightFirstItem, updateIsOpen, showAll]);


  const handlePrefix = (e) => {
    const prefix = e.target.value
    if (cleanList.current && showAll && prefix.length === 0) {
      openDropDown(cleanList.current)
      return
    }
    if (prefix.length > 0) {
